...
export const WorkSection = () => {
  ...
  const categories = useMemo(() => {
    ...
    mockData.projects.forEach(p => cats.add(p.category));
    return Array.from(cats);
  }, []);

  // Filter projects based on activeFilter
  const filteredProjects = useMemo(() => {
    if (activeFilter === "All") return mockData.projects;
    return mockData.projects.filter(p => p.category === activeFilter);
  }, [activeFilter]);
        ...
        >
          {categories.map((category, index) => (
            <motion.button
              ...
              className={`px-6 py-2.5 rounded-full font-medium transition-all text-sm sm:text-base ${
                activeFilter === category
                  ? "bg-gradient-to-r from-purple-500 to-blue-600 text-white shadow-lg shadow-purple-500/30"
                  : "bg-gray-900/60 text-gray-300 border border-gray-700/50 hover:border-purple-500/40"
              }`}
